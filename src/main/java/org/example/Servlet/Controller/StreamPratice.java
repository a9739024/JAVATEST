...
public class StreamPratice {
    ...
    void test_01 () {
        ...
        * */
        List<Transaction> transactions = transactionList.stream().filter(x->x.getYear() == 2011)
    ...
    void test_02 () {
         ...
         * 交易员都在哪些不同的城市工作过？
         * */
        List<String> areas = transactionList.stream().map(x->x.getTrader().getCity()).distinct().collect(Collectors.toList());
    ...
    void test_03 () {
         ...
         * */
        List<Trader> traders = transactionList.stream().map(Transaction::getTrader).distinct().filter(x->x.getCity().equals("Cambridge"))
    ...
    void test_04 () {
         ...
         * */
        List<String> names = transactionList.stream().map(x->x.getTrader().getName()).distinct().sorted(String::compareTo).collect(Collectors.toList());
    ...
    void test_05 () {
         ...
         * 有没有交易员在Milan工作。
         * */
        List<Trader> milans = transactionList.stream().map(Transaction::getTrader).distinct().filter(x->x.getCity().equals("Milan")).collect(Collectors.toList());
    ...
    void test_06 () {
         ...
         * */
        double total = transactionList.stream().filter(x->x.getTrader().getCity().equals("Cambridge")).mapToDouble(Transaction::getValue).sum();
    ...
    public static void main(String[] Args){
        ...
        List<Integer> biggerTen = list.stream().filter(x->x%2 == 0).collect(Collectors.toList());
        ...
        System.out.println("===============================");
        double sum = sh.stream().filter(x->x.getPingFen() > 4.5).filter(x->x.getCaiPin().stream().anyMatch(y->y.startsWith("A")))
                        .filter(x->x.getCaiPin().stream().anyMatch(y->y.startsWith("B"))).mapToDouble(ShangHu::getPingJiaShu).sum();
        ...
        double avg = avgList.stream().filter(x->x%2 == 0).mapToDouble(x->x).average().getAsDouble();
        ...
        String reB = reDupli.stream().distinct().map(String::valueOf).collect(Collectors.joining(","));
        ...
        Map<String,Integer> users = nameString.stream().filter(x->Integer.parseInt(x.split("，")[1]) >= 24).collect(Collectors.toMap(x->x.split("，")[0], x->Integer.valueOf(x.split("，")[1])));
        ...
        Stream<String> man = manList.stream().filter(x->x.split(",")[0].length() == 3).limit(2);
        Stream<String> woman = womenList.stream().filter(x->x.split(",")[0].equals("杨")).skip(1);
        List<Actor> actors = Stream.concat(man,woman).map(x->new Actor(x.split(",")[0],Integer.valueOf(x.split(",")[1]))).collect(Collectors.toList());
